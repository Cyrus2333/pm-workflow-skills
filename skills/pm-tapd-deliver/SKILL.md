...
7. 按需读取当前项目 `AGENTS.md`、产品文档、已确认的需求定义、分级、PRD、交付清单和已有 TAPD 对象。
...
- 已有背景、目标、范围和等级：直接准备 TAPD 预览；
- 需求清楚但等级缺失：建议 `pm-requirement-grade`；用户明确指定等级时直接复用；
- 需求本身不清楚：建议 `pm-requirement-define` 并停止；
- 用户明确“快速占位”：至少取得可解析 workspace、名称和等级；背景 / 目标 / 范围只写已知信息，不能补造。
...
业务允许的主 MCP 操作限于 Story / Task 的读取、字段与 workflow 读取、`create_story_or_task` 和 `update_story_or_task`。附件仅允许列表、上传与回读。`enabled_tools` / `disabled_tools` 属于部署配置，不由本 Skill 修改或假定。

## 默认输出

未确认前输出《TAPD 写入预览》，完整说明 workspace、目标对象、Description、每个字段的来源、状态计划、查重结果、能力状态、附件和“本轮不会执行什么”。

确认并执行后输出《TAPD 写入结果》：已完成动作、逐项 Readback 断言、对象 ID / 可用链接、失败或未执行动作、尚缺事实与下一步。

## 完成标准

- 所有写操作之前均有同一轮完整预览和用户明确确认；
- Story / Task 使用实时 workspace、字段、枚举和 workflow 事实，不依赖固定 custom field 编号；
- Story 标题、三段 Description、查重、状态计划和 Readback 符合交付协议；
