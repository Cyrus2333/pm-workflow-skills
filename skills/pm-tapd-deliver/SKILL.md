...
3. 按字段来源优先级计算写入值：用户明确值 > 当前确认事实 > 可靠用户 / 项目默认 > 推荐 > 通用默认。Story 的“分类”和“标签”是必填写入计划字段：每次创建或更新前均须从实时 Story schema 唯一解析，缺失、歧义或非法即停止；不得以“未分类”或空标签兜底。
...
- Story 已预览标题、Description、分类、标签、查重、状态计划和 Readback 符合交付协议；分类和标签均来自实时 schema、已在 dry-run form 显示、并在 Readback 中断言；成功后已报告 Necessity Check，未把推荐当作 Task 创建授权；
