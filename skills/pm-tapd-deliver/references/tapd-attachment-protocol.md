...
只有 list、upload、Readback 同时可用，才可执行用户明确要求的附件上传。缺失时在写入前让用户在“仅执行 Story / Task”“跳过附件”“本次全部停止”之间选择；不得先创建再告知附件不可用。若附件仅是已确认动作包的后置动作，上传能力中途失败时停止后续附件，但可保留已成功核验的 Story / Task 结果。
...
先读取用户明确指定的需求包目录或交付清单，不扫描其他目录猜文件。按“目标 Task → 实际文件 → 映射证据”建立清单，并标记 `exact / derived_summary / substitute / missing`。缺少 PRD、原型、spec、上线文档等用户点名产物时，必须在预览中列出缺口；任何替代上传都要单独确认。不要把 README、实现记录或验收清单静默换作其他交付物。
