...
- 修正首次同步时整文件覆盖造成的协议缺失，恢复历史补录、动作包 / 断点续作、差异分级、日期证据和附件映射规则，并与分类 / 标签更新合并。
- 合并本地最新适配器增量：负责人末尾分号规范化、Story 日期字段回读投影，以及 TAPD `StepPrevious` / `StepNext` 工作流字段兼容。
