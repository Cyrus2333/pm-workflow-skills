...
- `pm-tapd-deliver` 传输层改为官方 OpenAPI 窄适配器 `scripts/pm_tapd.py`，不再依赖 MCP 或第三方 CLI。支持 Story/Task 预检、查重、dry-run、写入、Readback、Story 官方 workflow 证明、真实 mention，以及标准附件 list/upload/Readback。Task workflow 在官方 `system=task` 被拒绝时回退到文档化状态图；附件 upload 使用社区合同，TAPD 拒绝则停止且不伪造。
